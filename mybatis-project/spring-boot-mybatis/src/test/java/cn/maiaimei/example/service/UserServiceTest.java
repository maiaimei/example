...
class UserServiceTest extends TestBase {
    ...
    void testPageQuery() {
        ...
        PaginationResult<UserResponse> response = readFileAsObject(PAGE_QUERY_RESPONSE, new TypeReference<PaginationResult<UserResponse>>() {
        });
        List<User> result = readFileAsObject(PAGE_QUERY_RESULT, new TypeReference<List<User>>() {
        });
        when(userMapper.list(any())).thenReturn(result);
        PaginationResult<UserResponse> actualResponse = userService.pageQuery(request, 1, 3);
